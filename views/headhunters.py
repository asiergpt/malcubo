                        ...
                        with col_personas:
                            ...
                            if pd.notna(personas_clave_str) and str(personas_clave_str).strip() != '' and str(personas_clave_str).lower() not in ['nan', 'none']:
                                with st.popover("👤 Contactos", use_container_width=True):
                                    ...
                                    # 1. Separamos por la barra vertical "|" en lugar del punto y coma
                                    lista_personas = [p.strip() for p in str(personas_clave_str).split('|') if p.strip()]
                                    
                                    # 2. Creamos un contenedor HTML flex para alinear las píldoras
                                    html_badges = "<div style='display: flex; flex-wrap: wrap; gap: 8px; margin-top: 5px;'>"
                                    
                                    for p_nombre in lista_personas:
                                        p_nombre_formateado = p_nombre.title()
                                        # 3. Dibujamos el nombre dentro de un <span> para que NO sea clickable
                                        html_badges += f"""
                                        <span style='
                                            background-color: #E67E2215; 
                                            color: #E67E22; 
                                    ...
                                    # 4. Renderizamos el HTML en Streamlit

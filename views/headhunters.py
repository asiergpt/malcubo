                            ...
                            if pd.notna(personas_clave_str) and str(personas_clave_str).strip() != '' and str(personas_clave_str).lower() not in ['nan', 'none']:
                                ...
                                with st.popover("👤👤 Contactos", use_container_width=True):
                                    ...
                                    for idx, p_nombre in enumerate(lista_personas):
                                        ...
                                        # Renderizamos como botón NORMAL (sin disabled=True).
                                        # Al no poner un "if st.button(...):", el botón se verá activo pero no hará nada.
                                        st.button(f"👤 {p_nombre_formateado}", key=f"btn_pc_hh_{index}_{idx}", use_container_width=True)
                            else:
                                # Comportamiento idéntico a Entidades cuando no hay contactos
                                with st.popover("👤👤 Contactos", use_container_width=True):
                                    st.info("Aún no tenemos contactos mapeados para este headhunter.")
    else:

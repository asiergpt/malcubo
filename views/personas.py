...
def show_personas(df_main, df_alumni):
    ...
    with st.sidebar:
        ...
        if not df_alumni.empty:
            # --- FILTRO JERARQUÍA (A PRUEBA DE ERRORES) ---
            if 'jerarquía' in df_alumni.columns:
                jerarquias = sorted([str(j).strip() for j in df_alumni['jerarquía'].dropna().unique() if str(j).strip().lower() not in ['nan', '-', '']])
            else:
                jerarquias = []
            ...
            st.session_state.f_personas_jerarquia = st.multiselect("Nivel Jerárquico", jerarquias, default=valores_jerarquia_seguros)
            
            # --- FILTRO FUNCIÓN (A PRUEBA DE ERRORES) ---
            if 'función' in df_alumni.columns:
                funciones = sorted([str(f).strip() for f in df_alumni['función'].dropna().unique() if str(f).strip().lower() not in ['nan', '-', '']])
            else:
                funciones = []
            ...
            st.session_state.f_personas_funcion = st.multiselect("Función", funciones, default=valores_funcion_seguros)
